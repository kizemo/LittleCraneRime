...
5. 任务栏托盘图标：仅一个小鹅图标（若仍有幽灵图标，注销一次 Windows）。
...
2. **溢出区多「小鹅算法服务」**：`PurgeAllLegacyWeaselTrayIcons()`（GUID + 历史 uID）；安装时 `/purge-tray`；`Create` 仅在可见时 `NIM_ADD`。
...
| 设置中 10 个「小鹅算法服务」 | `NotifyIconSettings` 未清理成功 |
