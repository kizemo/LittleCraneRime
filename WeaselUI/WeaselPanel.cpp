...
void UiPanelLog(const wchar_t* detail) {
  WeaselAppendLogW(L"weasel.ui.log", L"panel", detail);
}
...
WeaselPanel::WeaselPanel(weasel::UI& ui)
    : m_layout(NULL),
      ...
      _DeleteCandidateCallback(ui.deleteCandidateCallback()),
      _m_gdiplusToken(0) {
...
static bool IsInvalidInputRect(const RECT& rc) {
  return rc.left <= 0 && rc.top <= 0 && rc.right <= 8 && rc.bottom <= 20;
}

static RECT GetCaretOrCursorRect() {
  RECT rc = {0, 0, 8, 20};
  ...
  if (hwnd &&
    ...
    rc.top = gti.rcCaret.bottom;
    rc.right = gti.rcCaret.right;
    rc.bottom = gti.rcCaret.bottom + 20;
    ClientToScreen(gti.hwndCaret, (LPPOINT)&rc.left);
    ClientToScreen(gti.hwndCaret, (LPPOINT)&rc.right);
    return rc;
  }
  POINT pt = {0, 0};
  if (hwnd && GetCaretPos(&pt)) {
...
void WeaselPanel::Refresh() {
  if (m_status.composing && !m_ctx.cinfo.candies.empty()) {
    wchar_t buf[96];
    swprintf_s(buf, L"refresh enter font=%d candies=%u",
               m_style.font_point, (unsigned)m_ctx.cinfo.candies.size());
    UiPanelLog(buf);
  }
  ...
  bool inline_no_candidates = (m_style.inline_preedit && m_candidateCount == 0 &&
                               !m_status.composing && m_ctx.preedit.empty()) &&
                              !show_tips;
  ...
  if (!hide_candidates || inline_no_candidates) {
    if (IsInvalidInputRect(m_inputPos) &&
        (m_status.composing || !m_ctx.cinfo.candies.empty())) {
      RECT caret = GetCaretOrCursorRect();
      m_inputPos = caret;
    ...
    _ResizeWindow();
    _RepositionWindow();
    const bool composing_with_candidates =
        m_status.composing && m_candidateCount > 0;
    const bool need_redraw =
        (m_ctx != m_octx) || composing_with_candidates;
    if (composing_with_candidates) {
      CSize sz = m_layout->GetContentSize();
      wchar_t buf[160];
      swprintf_s(buf,
                 L"refresh hide=%d size=%dx%d font=%d redraw=%d",
                 hide_candidates ? 1 : 0, sz.cx, sz.cy, m_style.font_point,
                 need_redraw ? 1 : 0);
      UiPanelLog(buf);
    }
    if (need_redraw) {
...
int WeaselPanel::HitTestCandidateIndex(CPoint point) {
  if (!m_layout)
    return -1;
...
void WeaselPanel::DeleteCandidateAt(int index) {
  if (hide_candidates || index < 0 ||
      index >= static_cast<int>(m_candidateCount))
    return;
  wchar_t buf[96];
  swprintf_s(buf, L"delete candidate index=%d callback=%d", index,
             _DeleteCandidateCallback ? 1 : 0);
  UiPanelLog(buf);
  if (_DeleteCandidateCallback)
    _DeleteCandidateCallback(static_cast<size_t>(index));
}

void WeaselPanel::DeleteCandidateAtPoint(CPoint client_point) {
  if (hide_candidates || m_candidateCount == 0) {
    UiPanelLog(L"delete skipped: hidden or empty");
    return;
  }
  const int index = HitTestCandidateIndex(client_point);
  wchar_t buf[96];
  swprintf_s(buf, L"delete at point index=%d", index);
  UiPanelLog(buf);
  if (index >= 0)
    DeleteCandidateAt(index);
}

LRESULT WeaselPanel::OnRightClickedDown(UINT uMsg,
                                          WPARAM wParam,
                                          LPARAM lParam,
                                          BOOL& bHandled) {
  CPoint point(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
  const int index = HitTestCandidateIndex(point);
  wchar_t buf[96];
  swprintf_s(buf, L"RBUTTONDOWN index=%d hide=%d count=%u", index,
             hide_candidates ? 1 : 0, m_candidateCount);
  UiPanelLog(buf);
  if (!hide_candidates && index >= 0)
    DeleteCandidateAt(index);
  bHandled = TRUE;
...
LRESULT WeaselPanel::OnNcHitTest(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
  CPoint point(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
  ScreenToClient(&point);
  if (!hide_candidates && m_layout) {
    CRect content = m_layout->GetContentRect();
    if (content.PtInRect(point)) {
      bHandled = TRUE;
      return HTCLIENT;
    }
  }
...
LRESULT WeaselPanel::OnDeletePromptMessage(UINT uMsg,
                                           WPARAM wParam,
                                           LPARAM lParam,
                                           BOOL& bHandled) {
  (void)uMsg;
...
LRESULT WeaselPanel::OnContextMenu(UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam,
                                   BOOL& bHandled) {
  (void)uMsg;
...
HBITMAP CreateTransparentBitmap(HDC hdc, int width, int height, void** bits) {
  BITMAPINFO bmi = {};
...
void WeaselPanel::DoPaint(CDCHandle dc) {
  ...
  if (rcw.Width() <= 0 || rcw.Height() <= 0) {
    if (m_status.composing && !m_ctx.cinfo.candies.empty()) {
      wchar_t buf[128];
      swprintf_s(buf, L"paint skip zero-size font=%d layout=%d candies=%zu",
                 m_style.font_point, (int)m_style.layout_type,
                 m_ctx.cinfo.candies.size());
      UiPanelLog(buf);
    }
    return;
  }
  // prepare memDC with fully transparent 32bpp bitmap
  CDCHandle hdc = ::GetDC(m_hWnd);
  void* bits = nullptr;
  HBITMAP memBitmap =
      CreateTransparentBitmap(hdc, rcw.Width(), rcw.Height(), &bits);
  if (!memBitmap) {
    ReleaseDC(hdc);
    return;
  }
  if (bits)
    memset(bits, 0, static_cast<size_t>(rcw.Width()) * rcw.Height() * 4);
  ...
  if (!hide_candidates) {
    ...
    if (!drawn && !m_status.composing && m_ctx.preedit.empty() &&
        m_ctx.cinfo.candies.empty())
      ShowWindow(SW_HIDE);
  }
  if (!drawn && hide_candidates) {
    ::DeleteDC(memDC);
  ...
  if (m_status.composing && m_candidateCount > 0) {
    CRect wr;
    GetWindowRect(&wr);
    wchar_t buf[160];
    swprintf_s(buf,
               L"paint drawn=%d visible=%d hide=%d candies=%u %dx%d @%d,%d",
               drawn ? 1 : 0, visible ? 1 : 0, hide_candidates ? 1 : 0,
               (unsigned)m_candidateCount, rcw.Width(), rcw.Height(), wr.left,
               wr.top);
    UiPanelLog(buf);
  }
...
void WeaselPanel::ClearLayeredSurface() {
  if (!IsWindow())
    return;
  GetClientRect(&rcw);
  if (rcw.Width() <= 0 || rcw.Height() <= 0)
...
void WeaselPanel::MoveTo(RECT const& rc) {
  ...
  RECT input_rc = rc;
  if (IsInvalidInputRect(input_rc) &&
      (m_status.composing || !m_ctx.cinfo.candies.empty()))
    input_rc = GetCaretOrCursorRect();
  ...
  if (m_style.ascii_tip_follow_cursor && m_ctx.empty() &&
    ...
    RedrawWindow();
  } else if (!(input_rc.left == m_inputPos.left &&
               input_rc.bottom != m_inputPos.bottom &&
               abs(input_rc.bottom - m_inputPos.bottom) < 6) ||
