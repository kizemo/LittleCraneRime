...
// High bit of TrayCommand lParam: user explicitly toggled ascii (langbar / CLI).
// Compartment / legacy TSF paths must not set this bit.
constexpr DWORD WEASEL_TRAY_USER_INITIATED = 0x80000000u;
...
struct RequestHandler {
  ...
  virtual bool DeleteCandidateOnCurrentPage(size_t index,
                                            DWORD session_id,
                                            EatLine eat) {
    return false;
  }
  virtual BOOL CommitText(LPWSTR buffer, DWORD session_id, EatLine eat) {
    return FALSE;
  }
  ...
  virtual void ToggleOption(DWORD session_id,
                            const std::string& opt,
                            EatLine eat = nullptr) {}
  virtual void Respond(DWORD session_id, EatLine eat) {}
...
class Client {
 public:
  ...
  // 测试连接
  bool Echo();
  // 确保 IPC 会话已建立（StartSession + Echo 校验）
  bool EnsureSession();
  ...
  // 用户主动切换中英文（带 WEASEL_TRAY_USER_INITIATED 授权位）
