...
class PipeChannel : public PipeChannelBase {
 ...
 public:
  ...
  bool HandleResponseData(_TyHandler const& handler) {
    ...
    LPWSTR start = reinterpret_cast<LPWSTR>(buffer.get());
    UINT len = static_cast<UINT>(buff_size * sizeof(char) / sizeof(wchar_t));
    const wchar_t marker[] = L"action=";
    for (UINT i = 0; i + 6 < len; ++i) {
      if (start[i] == marker[0] && wcsncmp(start + i, marker, 6) == 0) {
        start += i;
        len -= i;
        break;
      }
    }
    return handler(start, len);
  }
