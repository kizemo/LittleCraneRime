...
class RimeWithWeaselHandler : public weasel::RequestHandler {
 public:
  ...
  virtual bool DeleteCandidateOnCurrentPage(size_t index,
                                            WeaselSessionId ipc_id,
                                            EatLine eat);
  virtual BOOL CommitText(LPWSTR buffer, WeaselSessionId ipc_id, EatLine eat);
  ...
  virtual void SetOption(WeaselSessionId ipc_id,
                         ...
                         bool val);
  virtual void ToggleOption(WeaselSessionId ipc_id,
                            const std::string& opt,
                            EatLine eat = nullptr);
  virtual void Respond(WeaselSessionId ipc_id, EatLine eat);
  virtual void UpdateColorTheme(BOOL darkMode);

  void OnUpdateUI(std::function<void()> const& cb);

  void SelectSchema(const std::string& schema_id);
  void ToggleOption(const std::string& opt);
  std::string GetCurrentSchemaId();
  bool GetOption(const std::string& opt);
  std::vector<std::pair<std::string, std::string>> GetSchemaList();
  void RequestCommitText(const std::wstring& text);
 ...
 private:
  ...
  DWORD m_pid;
  std::map<DWORD, HWND> m_client_notify_hwnd;
};
