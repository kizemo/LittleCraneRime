...
BOOL CSystemTray::Create(HINSTANCE hInst,
  ...
  m_tnd.uID = uID;
  m_uMenuId = uMenuID ? uMenuID : uID;
  m_tnd.hIcon = icon;
  m_tnd.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP;
  if (pNotifyGuid) {
    m_tnd.guidItem = *pNotifyGuid;
    m_tnd.uFlags |= NIF_GUID;
  ...
  if (!m_bHidden) {
    if (pNotifyGuid)
      weasel_tray::PurgeWeaselTrayIconByGuid();
    bResult = Shell_NotifyIcon(NIM_ADD, &m_tnd);
    m_bShowIconPending = m_bHidden = m_bRemoved = !bResult;
    weasel_tray::TrayLog(L"Create NIM_ADD", m_tnd.uID, m_tnd.hWnd, bResult);
  } else {
    weasel_tray::TrayLog(L"Create deferred", m_tnd.uID, m_tnd.hWnd, TRUE);
  }
...
BOOL CSystemTray::AddIcon() {
  ...
  if (m_bEnabled) {
    static bool s_guid_purged = false;
    if (m_tnd.guidItem.Data1 != 0 && !s_guid_purged) {
      weasel_tray::PurgeWeaselTrayIconByGuid();
      s_guid_purged = true;
    }
    m_tnd.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP;
    if (m_tnd.guidItem.Data1 != 0)
      m_tnd.uFlags |= NIF_GUID;
    BOOL ok = Shell_NotifyIcon(NIM_ADD, &m_tnd);
    weasel_tray::TrayLog(L"NIM_ADD", m_tnd.uID, m_tnd.hWnd, ok);
    if (!ok)
...
BOOL CSystemTray::RemoveIcon() {
  ...
  if (m_tnd.guidItem.Data1 != 0)
    m_tnd.uFlags = NIF_GUID;
  BOOL ok = Shell_NotifyIcon(NIM_DELETE, &m_tnd);
  weasel_tray::TrayLog(L"NIM_DELETE", m_tnd.uID, m_tnd.hWnd, ok);
  if (ok)
...
BOOL CSystemTray::SetMenuDefaultItem(UINT uItem, BOOL bByPos) {
  ...
  HMENU hMenu = ::LoadMenu(m_hInstance, MAKEINTRESOURCE(m_uMenuId));
  if (!hMenu)
    return FALSE;
...
LRESULT CSystemTray::OnTrayNotification(WPARAM wParam, LPARAM lParam) {
  // With NIF_GUID the shell may use a different wParam than m_tnd.uID.
  if (wParam != m_tnd.uID && (m_tnd.guidItem.Data1 == 0))
    return 0L;
