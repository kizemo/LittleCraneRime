...
static int Run(LPTSTR lpCmdLine) {
  ...
  if (!wcscmp(L"/?", lpCmdLine) || !wcscmp(L"/help", lpCmdLine)) {
    ...
    if (LoadString(GetModuleHandle(NULL), IDS_STR_HELP, msg,
      ...
      MessageBox(NULL,
                 ...
                 L"/deployquiet	- Update Workspace (silent, no UI)\n"
  ...
  bool deployment_quiet = !wcscmp(L"/deployquiet", lpCmdLine);
  if (deployment_quiet) {
    return configurator.UpdateWorkspace(false);
  }
  ...
  bool phrase_entry = !wcscmp(L"/phrase", lpCmdLine);
  if (phrase_entry) {
    AllowSetForegroundWindow(ASFW_ANY);
    return configurator.CustomPhraseEntry();
  }

  bool common_phrase_entry = !wcscmp(L"/commonphrase", lpCmdLine);
  if (common_phrase_entry) {
    AllowSetForegroundWindow(ASFW_ANY);
    return configurator.CommonPhraseEdit();
  }

  bool hotkey_settings = !wcscmp(L"/hotkey", lpCmdLine);
  if (hotkey_settings) {
    AllowSetForegroundWindow(ASFW_ANY);
    return configurator.HotkeySettings();
  }
