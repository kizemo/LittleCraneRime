...
rem -------------------------------------------------------------------------
if %build_weasel% == 1 (
  if not exist output\data\rime_ice.schema.yaml (
    set build_data=1
  )
...
rem ---------------------------------------------------------------------------
:build_data
  if not exist plum\rime-install (
  ...
  if not exist output\data\preview mkdir output\data\preview
  if not defined WEASEL_BUNDLED_RECIPES set WEASEL_BUNDLED_RECIPES=iDvel/rime-ice:others/recipes/full
  set plum_dir=plum
  set rime_dir=output/data
  set WSLENV=plum_dir:rime_dir
  bash plum/rime-install %WEASEL_BUNDLED_RECIPES%
  if errorlevel 1 goto error
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts\patch-rime-ice-config.ps1
  if errorlevel 1 goto error
  exit /b
