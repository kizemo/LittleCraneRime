...
std::string CustomPhraseFileHeader(const std::wstring& filename) {
...
std::wstring NormalizeDeletionCode(const std::wstring& code) {
  std::wstring result;
...
std::wstring ExtractDeletionCode(RimeApi* api,
                                 RimeSessionId session_id,
                                 RimeContext* ctx,
                                 size_t index) {
  if (ctx && static_cast<int>(index) < ctx->menu.num_candidates) {
    if (ctx->menu.candidates[index].comment) {
      std::wstring comment =
          NormalizeDeletionCode(u8tow(ctx->menu.candidates[index].comment));
      if (!comment.empty() && comment[0] == L'~')
        comment.erase(0, 1);
      if (!comment.empty())
        return comment;
    }
  }
  if (api && api->get_input) {
    if (const char* raw = api->get_input(session_id)) {
      if (*raw) {
        std::wstring input = NormalizeDeletionCode(u8tow(raw));
        if (!input.empty())
          return input;
      }
    }
  }
  if (ctx && ctx->composition.preedit)
    return NormalizeDeletionCode(u8tow(ctx->composition.preedit));
  return L"";
}

bool PhraseCodeMatchLine(const std::string& line,
                         const std::wstring& phrase,
                         const std::wstring& code) {
  auto tab = line.find('\t');
  if (tab == std::string::npos || line.empty() || line[0] == '#')
    return false;
  std::wstring col_a = u8tow(line.substr(0, tab));
  std::wstring col_b = u8tow(line.substr(tab + 1));
  auto tab2 = col_b.find(L'\t');
  if (tab2 != std::wstring::npos)
    col_b = col_b.substr(0, tab2);
  const bool a_is_code =
      !col_a.empty() &&
      std::all_of(col_a.begin(), col_a.end(), [](wchar_t c) {
        return iswalnum(c) || c == L'_' || c == L'-' || c == L'\'';
      });
  std::wstring line_phrase = a_is_code ? col_b : col_a;
  std::wstring line_code = a_is_code ? col_a : col_b;
  if (!code.empty())
    return line_phrase == phrase && line_code == code;
  return line_phrase == phrase;
}

bool RemoveCustomPhraseEntry(const std::wstring& phrase,
                             const std::wstring& code) {
  if (phrase.empty())
    return false;
  ...
  for (const wchar_t* fname :
       {L"custom_phrase.txt", L"custom_phrase_double.txt"}) {
    const fs::path path = user_dir / fname;
    if (!fs::exists(path))
      continue;
    ...
    while (std::getline(in, line)) {
      if (!line.empty() && line.back() == '\r')
        line.pop_back();
      if (!code.empty() && PhraseCodeMatchLine(line, phrase, code)) {
        file_removed = true;
        continue;
      }
      kept.push_back(line);
    }
    if (!file_removed && code.empty()) {
      int phrase_hits = 0;
      for (const auto& row : kept) {
        ...
        if (tab == std::string::npos)
          continue;
        std::wstring col_a = u8tow(row.substr(0, tab));
        std::wstring col_b = u8tow(row.substr(tab + 1));
        auto tab2 = col_b.find(L'\t');
        if (tab2 != std::wstring::npos)
          col_b = col_b.substr(0, tab2);
        const bool a_is_code =
        ...
        const std::wstring line_phrase = a_is_code ? col_b : col_a;
        if (line_phrase == phrase)
          ++phrase_hits;
      }
      if (phrase_hits == 1) {
    ...
    if (!file_removed)
      continue;
    fs::path tmp(path.wstring() + L".weasel.tmp");
    ...
    any_removed = true;
    if (rime_api && rime_api->deploy_config_file) {
      const std::string file_name = wtou8(std::wstring(fname));
      rime_api->deploy_config_file(file_name.c_str(), nullptr);
    }
  }
  return any_removed;
}

bool RemoveUserDictEntry(RimeSessionId session_id,
                         const std::wstring& phrase,
                         const std::wstring& code) {
  if (phrase.empty() || code.empty())
    return false;
  RimeModule* levers = rime_api ? rime_api->find_module("levers") : nullptr;
  if (!levers)
    return false;
  auto* api = (RimeLeversApi*)levers->get_api();
  if (!api || !api->import_user_dict)
    return false;
  const char* dict_name = "rime_ice";
  RIME_STRUCT(RimeStatus, status);
  if (session_id && rime_api->get_status(session_id, &status) &&
      status.schema_id) {
    RimeConfig schema_config = {};
    if (rime_api->schema_open(status.schema_id, &schema_config)) {
      char user_dict[256] = "";
      if (rime_api->config_get_string(&schema_config, "translator/user_dict",
                                      user_dict, sizeof(user_dict)) &&
          user_dict[0]) {
        dict_name = user_dict;
      } else {
        dict_name = status.schema_id;
      }
  ...
  namespace fs = std::filesystem;
  const fs::path tmp =
      fs::temp_directory_path() / L"weasel_delete_user_dict_entry.txt";
  {
    std::ofstream out(tmp, std::ios::binary | std::ios::trunc);
    out << "# Rime user dictionary import\n";
    out << wtou8(phrase) << "\t" << wtou8(code) << "\t-1\n";
  }
  const int n = api->import_user_dict(dict_name, wtou8(tmp.wstring()).c_str());
  return n > 0;
}

void SetAsciiModeOnSession(RimeSessionId session_id, bool val) {
  rime_api->set_option(session_id, "ascii_mode", Bool(val));
  rime_api->set_option(session_id, "ascii_punct", Bool(val));
}
...
namespace {
void CommitTextViaClipboard(const std::wstring& text) {
  if (text.empty() || !OpenClipboard(NULL))
    return;
  HGLOBAL clipboard_backup = NULL;
  if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
    HGLOBAL src = GetClipboardData(CF_UNICODETEXT);
    if (src) {
      const SIZE_T size = GlobalSize(src);
      clipboard_backup = GlobalAlloc(GMEM_MOVEABLE, size);
      if (clipboard_backup) {
        void* dst = GlobalLock(clipboard_backup);
        ...
        if (s)
          GlobalUnlock(src);
        if (dst)
          GlobalUnlock(clipboard_backup);
        if (!dst || !s) {
          GlobalFree(clipboard_backup);
  ...
  if (!hg) {
    if (clipboard_backup)
      SetClipboardData(CF_UNICODETEXT, clipboard_backup);
    CloseClipboard();
    return;
  }
  ...
  if (GetGUIThreadInfo(0, &gi) && gi.hwndCaret) {
    HWND root = GetAncestor(gi.hwndCaret, GA_ROOT);
    if (root && IsWindow(root)) {
      DWORD fg_tid = GetWindowThreadProcessId(root, NULL);
...
DWORD RimeWithWeaselHandler::AddSession(LPWSTR buffer, EatLine eat) {
  ...
  if (m_global_ascii_mode) {
    for (const auto& pair : m_session_status_map) {
      if (pair.first) {
        SetAsciiModeOnSession(session_id,
  ...
  if (eat) {
    _Respond(ipc_id, eat);
    // TSF StartSession only reads session id DWORD; style in pipe body is
    // overwritten before GetResponseData. Force style on first key response.
    session_status.__synced = false;
  }
  add_session = true;
  _UpdateUI(ipc_id);
...
DWORD RimeWithWeaselHandler::RemoveSession(WeaselSessionId ipc_id) {
  ...
  m_client_notify_hwnd.erase(ipc_id);
  m_active_session = 0;
  return 0;
}
...
BOOL RimeWithWeaselHandler::ProcessKeyEvent(KeyEvent keyEvent,
  ...
  const bool is_release = !!(keyEvent.mask & ibus::Modifier::RELEASE_MASK);
  const bool is_ctrl_space_toggle =
      !is_release && keyEvent.keycode == ibus::space &&
      (keyEvent.mask & ibus::CONTROL_MASK);
  const bool is_ctrl_shift_digit =
      !is_release && (keyEvent.mask & ibus::CONTROL_MASK) &&
      (keyEvent.mask & ibus::SHIFT_MASK) &&
      keyEvent.keycode >= '0' && keyEvent.keycode <= '9';
  const bool allow_ascii_key_toggle =
      is_ctrl_space_toggle || keyEvent.keycode == ibus::Caps_Lock;
  const bool allow_punct_key_toggle = is_ctrl_shift_digit;
  Bool was_ascii = rime_api->get_option(session_id, "ascii_mode");
  Bool was_punct = rime_api->get_option(session_id, "ascii_punct");
  Bool handled = rime_api->process_key(session_id, keyEvent.keycode,
                                       expand_ibus_modifier(keyEvent.mask));
  Bool now_ascii = rime_api->get_option(session_id, "ascii_mode");
  Bool now_punct = rime_api->get_option(session_id, "ascii_punct");
  if (was_ascii != now_ascii && !allow_ascii_key_toggle) {
    rime_api->set_option(session_id, "ascii_mode", was_ascii);
    rime_api->set_option(session_id, "ascii_punct", was_punct);
    now_ascii = was_ascii;
    now_punct = was_punct;
    wchar_t extra[96];
    swprintf_s(extra, L"block ascii flip keycode=%u mask=%u", keyEvent.keycode,
               keyEvent.mask);
    WeaselAsciiTraceMode(!!was_ascii, !!now_ascii, L"ascii-guard", extra);
  } else if (was_punct != now_punct && !allow_ascii_key_toggle &&
             !allow_punct_key_toggle) {
    rime_api->set_option(session_id, "ascii_punct", was_punct);
    now_punct = was_punct;
    wchar_t extra[96];
    swprintf_s(extra, L"block punct flip keycode=%u mask=%u", keyEvent.keycode,
               keyEvent.mask);
    WeaselAsciiTraceMode(!!was_punct, !!now_punct, L"punct-guard", extra);
  }
  if (was_ascii != now_ascii) {
    wchar_t extra[96];
    swprintf_s(extra, L"keycode=%u mask=%u", keyEvent.keycode, keyEvent.mask);
    WeaselAsciiTraceMode(!!was_ascii, !!now_ascii, L"rime-key", extra);
    SetAsciiModeOnSession(session_id, !!now_ascii);
    if (m_global_ascii_mode) {
      for (auto& pair : m_session_status_map) {
        if (pair.first != ipc_id)
          SetAsciiModeOnSession(to_session_id(pair.first), !!now_ascii);
      }
    }
  }
  ...
  if (!handled && !(keyEvent.mask & ibus::Modifier::RELEASE_MASK)) {
    ...
    if (isVimBackInCommandMode &&
      ...
      SetAsciiModeOnSession(session_id, true);
    }
  }
  _Respond(ipc_id, eat);
  _UpdateUI(ipc_id);
...
bool RimeWithWeaselHandler::DeleteCandidateOnCurrentPage(
    size_t index,
    WeaselSessionId ipc_id,
    EatLine eat) {
  if (m_disabled)
  ...
  if (rime_api->get_context(session_id, &ctx) && ctx.menu.num_candidates > 0 &&
      static_cast<int>(index) < ctx.menu.num_candidates) {
    if (ctx.menu.candidates[index].text)
      phrase = u8tow(ctx.menu.candidates[index].text);
    code = ExtractDeletionCode(rime_api, session_id, &ctx, index);
    rime_api->free_context(&ctx);
  }
  bool res = rime_api->delete_candidate_on_current_page(session_id, index);
  bool removed = false;
  if (!phrase.empty()) {
    if (!code.empty())
      removed = RemoveCustomPhraseEntry(phrase, code);
    if (!removed)
      removed = RemoveCustomPhraseEntry(phrase, L"");
    if (!removed && !code.empty())
      removed = RemoveUserDictEntry(session_id, phrase, code);
    if (removed && !code.empty()) {
      const std::string replay = wtou8(code);
      rime_api->clear_composition(session_id);
      rime_api->simulate_key_sequence(session_id, replay.c_str());
    }
  }
  DLOG(INFO) << "DeleteCandidate phrase=" << wtou8(phrase) << " code="
             << wtou8(code) << " rime=" << res << " fallback=" << removed;
  _Respond(ipc_id, eat);
  _UpdateUI(ipc_id);
  return res || removed;
}

BOOL RimeWithWeaselHandler::CommitText(LPWSTR buffer,
                                       WeaselSessionId ipc_id,
                                       EatLine eat) {
  if (m_disabled || !eat)
    return FALSE;
  ...
  while (bs.good()) {
    ...
    if (line == L".")
      break;
    const std::wstring kTextKey = L"text=";
    if (starts_with(line, kTextKey))
      text = line.substr(kTextKey.length());
  }
  if (text.empty())
    return FALSE;
  std::list<std::string> messages;
  messages.push_back("action=commit\n");
  messages.push_back(std::string("commit=") +
                     escape_string<char>(wtou8(text)) + "\n");
  messages.push_back("status.composing=0\n");
  messages.push_back(".\n");
  return std::all_of(messages.begin(), messages.end(),
                     [&eat](std::string& msg) { return eat(u8tow(msg)); });
}

void RimeWithWeaselHandler::RequestCommitText(const std::wstring& text) {
  if (text.empty())
    return;
  WeaselSessionId sid =
      m_active_session ? m_active_session : m_last_focused_session;

  std::vector<HWND> targets;
  auto add_target = [&](HWND hwnd) {
    if (!hwnd || !::IsWindow(hwnd))
      return;
    if (std::find(targets.begin(), targets.end(), hwnd) == targets.end())
      targets.push_back(hwnd);
  };
  if (sid) {
    auto it = m_client_notify_hwnd.find(sid);
    if (it != m_client_notify_hwnd.end())
      add_target(it->second);
  } else if (!m_client_notify_hwnd.empty()) {
    add_target(m_client_notify_hwnd.rbegin()->second);
  }

  const DWORD server_pid = GetCurrentProcessId();
  COPYDATASTRUCT cds = {};
  cds.dwData = WEASEL_COMMIT_COPYDATA_ID;
  cds.cbData = static_cast<DWORD>((text.size() + 1) * sizeof(wchar_t));
  cds.lpData = const_cast<wchar_t*>(text.c_str());

  for (HWND notify : targets) {
    const LRESULT sent = ::SendMessage(notify, WM_COPYDATA, server_pid,
                                       reinterpret_cast<LPARAM>(&cds));
    if (sent)
      return;
  }
  CommitTextViaClipboard(text);
}
...
void RimeWithWeaselHandler::_ReadClientInfo(WeaselSessionId ipc_id,
  ...
  while (bs.good()) {
    ...
    const std::wstring kNotifyHwndKey = L"session.client_notify_hwnd=";
    if (starts_with(line, kNotifyHwndKey)) {
      const std::wstring num = line.substr(kNotifyHwndKey.length());
      HWND hwnd = reinterpret_cast<HWND>(
          wcstoull(num.c_str(), nullptr, num.find(L'x') != std::wstring::npos ? 0 : 16));
      if (hwnd)
        m_client_notify_hwnd[ipc_id] = hwnd;
    }
  }
...
void RimeWithWeaselHandler::SetOption(WeaselSessionId ipc_id,
  ...
  if (opt == "ascii_mode") {
    wchar_t buf[128];
    swprintf_s(buf, L"SetOption ascii_mode=%d ipc_id=%lu", val ? 1 : 0,
               static_cast<unsigned long>(ipc_id));
    WeaselAsciiTrace(L"set-option", buf);
  }
  // from no-session client, not actual typing session
  if (!ipc_id) {
    if (m_global_ascii_mode && opt == "ascii_mode") {
      for (auto& pair : m_session_status_map)
        SetAsciiModeOnSession(to_session_id(pair.first), val);
    } else if (opt == "ascii_mode" && m_active_session) {
      SetAsciiModeOnSession(to_session_id(m_active_session), val);
      _UpdateUI(m_active_session);
      if (_UpdateUICallback)
        _UpdateUICallback();
    } else if (m_active_session) {
      rime_api->set_option(to_session_id(m_active_session), opt.c_str(), val);
    }
  } else if (opt == "ascii_mode") {
    SetAsciiModeOnSession(to_session_id(ipc_id), val);
    _UpdateUI(ipc_id);
    if (_UpdateUICallback)
      _UpdateUICallback();
  } else {
...
void RimeWithWeaselHandler::SelectSchema(const std::string& schema_id) {
  if (m_disabled || schema_id.empty())
    return;
  for (auto& pair : m_session_status_map) {
    RimeSessionId session_id = to_session_id(pair.first);
    if (session_id)
      rime_api->select_schema(session_id, schema_id.c_str());
  }
  if (m_session_status_map.empty()) {
...
void RimeWithWeaselHandler::ToggleOption(WeaselSessionId ipc_id,
                                         const std::string& opt,
                                         EatLine eat) {
  if (m_disabled || opt.empty() || !ipc_id)
    return;
  RimeSessionId session_id = to_session_id(ipc_id);
  if (!session_id) {
    WeaselAsciiTrace(L"set-option", L"ToggleOption skipped: no rime session");
    return;
  }
  Bool val = rime_api->get_option(session_id, opt.c_str());
  if (opt == "ascii_mode") {
    const bool next = !val;
    wchar_t buf[128];
    swprintf_s(buf,
               L"ToggleOption ascii_mode+ascii_punct %d->%d ipc_id=%lu",
               val ? 1 : 0, next ? 1 : 0,
               static_cast<unsigned long>(ipc_id));
    WeaselAsciiTrace(L"set-option", buf);
    SetAsciiModeOnSession(session_id, next);
  } else {
...
void RimeWithWeaselHandler::Respond(WeaselSessionId ipc_id, EatLine eat) {
  if (m_disabled || !ipc_id || !eat)
    return;
  _Respond(ipc_id, eat);
}

void RimeWithWeaselHandler::ToggleOption(const std::string& opt) {
  if (m_disabled || opt.empty())
    return;
  WeaselSessionId ipc_id = m_active_session;
  if (!ipc_id && !m_session_status_map.empty())
    ipc_id = m_session_status_map.begin()->first;
  if (!ipc_id)
    return;
  RimeSessionId session_id = to_session_id(ipc_id);
  Bool val = rime_api->get_option(session_id, opt.c_str());
  if (opt == "ascii_mode")
    SetAsciiModeOnSession(session_id, !val);
  else
    rime_api->set_option(session_id, opt.c_str(), !val);
  _UpdateUI(ipc_id);
  if (_UpdateUICallback)
...
std::string RimeWithWeaselHandler::GetCurrentSchemaId() {
  if (m_disabled)
    return m_last_schema_id;
  WeaselSessionId ipc_id = m_active_session;
  if (!ipc_id && !m_session_status_map.empty())
    ipc_id = m_session_status_map.begin()->first;
  if (!ipc_id)
  ...
  if (rime_api->get_current_schema(to_session_id(ipc_id), schema_id,
                                   sizeof(schema_id) - 1))
    return std::string(schema_id);
  return m_last_schema_id;
}

bool RimeWithWeaselHandler::GetOption(const std::string& opt) {
  if (m_disabled || opt.empty())
    return false;
  WeaselSessionId ipc_id = m_active_session;
  if (!ipc_id && !m_session_status_map.empty())
  ...
  if (!ipc_id)
    return false;
  return !!rime_api->get_option(to_session_id(ipc_id), opt.c_str());
}

std::vector<std::pair<std::string, std::string>>
RimeWithWeaselHandler::GetSchemaList() {
  std::vector<std::pair<std::string, std::string>> result;
  if (m_disabled || !rime_api)
    return result;
  RimeSchemaList list = {0};
  if (rime_api->get_schema_list(&list)) {
    for (size_t i = 0; i < list.size; ++i) {
      const char* id = list.list[i].schema_id ? list.list[i].schema_id : "";
      const char* name = list.list[i].name ? list.list[i].name : id;
...
bool RimeWithWeaselHandler::_Respond(WeaselSessionId ipc_id, EatLine eat) {
  ...
  if (rime_api->get_status(session_id, &status)) {
    ...
    if (m_global_ascii_mode &&
      ...
      for (auto& pair : m_session_status_map) {
        if (pair.first != ipc_id)
          SetAsciiModeOnSession(to_session_id(pair.first),
  ...
  if (rime_api->get_context(session_id, &ctx)) {
    ...
    if (is_composing) {
      wchar_t cbuf[128];
      swprintf_s(cbuf, L"ipc=%lu preedit_len=%d menu=%d",
                 static_cast<unsigned long>(ipc_id),
                 ctx.composition.preedit ? (int)strlen(ctx.composition.preedit)
                                         : 0,
                 ctx.menu.num_candidates);
      WeaselAppendLogW(L"weasel.server.log", L"compose", cbuf);
    }
    rime_api->free_context(&ctx);
  }
