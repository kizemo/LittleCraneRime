...
LONG WINAPI _UnhandledExceptionFilter(EXCEPTION_POINTERS* pExceptionPointers) {
  weasel_crash_diag::WriteCrashReport(pExceptionPointers);
  if (pExceptionPointers && pExceptionPointers->ExceptionRecord) {
    wchar_t crash_buf[160];
    swprintf_s(
        crash_buf, L"code=0x%08X addr=%p",
        pExceptionPointers->ExceptionRecord->ExceptionCode,
        pExceptionPointers->ExceptionRecord->ExceptionAddress);
    WeaselAppendLogW(L"weasel.tsf.log", L"crash", crash_buf);
  }
  ...
  switch (dwReason) {
    case DLL_PROCESS_ATTACH:
      g_hInst = hInstance;
      {
        ...
        swprintf_s(buf, L"weasel.dll loaded " WEASEL_VERSION L" path=%s", path);
        WeaselAppendLogW(L"weasel.tsf.log", L"startup", buf);
      }
      weasel_crash_diag::Install();
