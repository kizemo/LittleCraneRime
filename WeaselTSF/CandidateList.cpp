...
static bool ShouldShowWeaselPanel(const Context& ctx, const Status& status) {
  if (!status.composing)
    return false;
  if (!ctx.cinfo.candies.empty())
    return true;
  if (!ctx.aux.str.empty())
    return true;
  return false;
}
...
void CCandidateList::UpdateUI(const Context& ctx, const Status& status) {
  try {
    _UpdateUIImpl(ctx, status);
  } catch (...) {
    WeaselAppendLogW(L"weasel.ui.log", L"panel",
                     L"exception in UpdateUI");
  }
}

void CCandidateList::_UpdateUIImpl(const Context& ctx, const Status& status) {
  _ui->style() = _style;
  ...
  const BOOL show_panel = ShouldShowWeaselPanel(ctx, status) ? TRUE : FALSE;
  wchar_t buf[160];
  swprintf_s(buf, L"candies=%zu composing=%d pbShow=%d show=%d hwnd=%p font=%d",
             ctx.cinfo.candies.size(), status.composing ? 1 : 0,
             _pbShow ? 1 : 0, show_panel ? 1 : 0, _ui->GetHwnd(),
             _style.font_point);
  WeaselAppendLogW(L"weasel.ui.log", L"panel", buf);
...
void CCandidateList::UpdateStyle(const UIStyle& sty) {
  ...
  _ui->style() = sty;
  if (_tsf)
    _tsf->SyncLanguageBarTrayVisibility();
}
...
void CCandidateList::StartUI() {
  ...
  _ui->SetDeleteCandidateCallback([this](size_t index) {
    wchar_t buf[64];
    swprintf_s(buf, L"delete callback index=%zu", index);
    WeaselAppendLogW(L"weasel.ui.log", L"delete", buf);
    _tsf->_DeleteCandidateOnCurrentPage(index);
  });
  BOOL tsf_pb_show = _pbShow;
  if (!_ui_element_started) {
    pUIElementMgr->BeginUIElement(this, &tsf_pb_show, &uiid);
    _ui_element_started = true;
    _pbShow = tsf_pb_show;
  }
  _ui->style() = _style;
  _MakeUIWindow();
...
void CCandidateList::EndUI() {
  ...
  if (pThreadMgr) {
    ...
    if (FAILED(hr))
      return;
    if (emgr != NULL && _ui_element_started) {
      emgr->EndUIElement(uiid);
      _ui_element_started = false;
    }
  }
  _DisposeUIWindow();
}
...
size_t CCandidateList::GetHighlightedIndex() const {
  if (!_ui)
    return 0;
  return _ui->ctx().cinfo.highlighted;
}

void CCandidateList::_InstallMouseHook() {}

void CCandidateList::_UninstallMouseHook() {}
...
void WeaselTSF::_DeleteCandidateOnCurrentPage(size_t index) {
  ...
  if (!m_client.DeleteCandidateOnCurrentPage(index)) {
    wchar_t buf[64];
    swprintf_s(buf, L"DeleteCandidate failed index=%zu", index);
    WeaselAppendLogW(L"weasel.tsf.log", L"delete", buf);
    return;
  }
  _RequestIpcRefresh(TRUE, TRUE);
  if (_pEditSessionContext)
    _UpdateComposition(_pEditSessionContext);
  else if (com_ptr<ITfContext> ctx = _GetFocusedTfContext())
