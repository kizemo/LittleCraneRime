...
void WeaselTSF::_ScheduleTrayCommand(UINT wID) {
  _pendingTrayCommand = wID;
}

void WeaselTSF::_FlushPendingTrayCommand() {
  if (!_pendingTrayCommand)
    return;
  UINT cmd = _pendingTrayCommand;
  _pendingTrayCommand = 0;
  if (cmd == ID_WEASELTRAY_ENABLE_ASCII ||
      cmd == ID_WEASELTRAY_DISABLE_ASCII) {
    WeaselAppendLogW(L"weasel.tsf.log", L"tray",
                     L"drop legacy ascii tray cmd (use SetAsciiMode)");
    return;
  }
  wchar_t logbuf[64];
  swprintf_s(logbuf, L"flush tray cmd id=%u", cmd);
  WeaselAppendLogW(L"weasel.tsf.log", L"tray", logbuf);
  if (cmd == ID_WEASELTRAY_QUICK_BAR) {
    m_client.TrayCommand(cmd);
  } else {
    _HandleLangBarMenuSelect(cmd);
  }
}

HRESULT WeaselTSF::_HandleCompartment(REFGUID guidCompartment) {
  if (_isSyncingCompartment) {
    WeaselAppendLogW(L"weasel.tsf.log", L"compartment",
                     L"skip reentrant compartment event");
    return S_OK;
  }

  if (ShiftComboGuard::ShouldIgnoreAsciiSync()) {
    WeaselAppendLogW(L"weasel.tsf.log", L"compartment",
                     L"skip ascii sync during shift combo");
    return S_OK;
  }
  ...
  if (IsEqualGUID(guidCompartment, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE)) {
    if (_isToOpenClose) {
      ...
      WeaselAppendLogW(L"weasel.tsf.log", L"compartment",
                       L"ignore KEYBOARD_OPENCLOSE reverse sync");
    }
  } else if (IsEqualGUID(guidCompartment,
                         GUID_COMPARTMENT_KEYBOARD_INPUTMODE_CONVERSION)) {
    // Windows may flip INPUTMODE_CONVERSION on Shift+symbol before TSF sees the
    // combo key. Reverse-syncing compartment -> Rime ascii_mode was the root
    // cause of accidental Chinese/English toggles (see weasel.tsf.log tray
    // cmd 40013/40014 without any "skip ascii sync during shift combo").
    // Rime status remains authoritative; only Rime -> compartment via
    // _UpdateLanguageBar.
    WeaselAppendLogW(L"weasel.tsf.log", L"compartment",
                     L"ignore INPUTMODE_CONVERSION reverse sync");
  }
  return S_OK;
}
