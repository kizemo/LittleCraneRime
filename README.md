...
Rime（中州韵、小狼毫、小企鹅等平台实现）是一款由 [佛振](https://github.com/lotem) 主导开发的开源输入法引擎。它的设计哲学强调**配置即一切**：通过几份 `yaml` 配置文件，用户可以自由地定义输入法的一切行为——这种高度可定制性，恰恰也是它最让普通用户望而却步的地方。
...
- **基线版本**：[rime/weasel 0.18.99](https://github.com/rime/weasel)（同步至 0.18.99 的官方修复）
...
下表给出本仓库**相对上游 0.18.99 的可感知差异**，按"用户可感知"程度由高到低排序。
...
A: 上游 0.18.99 是技术基线，本仓库在其上叠加了可视化设置体系与稳定性修订。可独立使用，也可以无损回滚到上游。
